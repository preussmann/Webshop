<?php
	...
	if( isset( $_POST['AGB']) ) {
		...
		$Bestellung = $Verbindung->stmt_init() ;
		$Bestellung->prepare('
			INSERT INTO bestellung ( Kunde, Zahlungsart, Lieferadresse )
			VALUES ( ?, ?, ? ) ;
		');
		$Bestellung->bind_param('iss', $_SESSION['Kunde'], $_POST['Zahlungsart'], $_POST['Lieferadresse'] );
		$Bestellung->execute() ;
		$BestellNr = $Verbindung->insert_id ;
		// Alle zugehörigen Posten eintragen
		$Posten = $Verbindung->stmt_init() ;
		$Posten->prepare('
			INSERT INTO posten ( Bestellung, Artikel, Menge, Preis ) 
			VALUES ( ?, ?, ?, ? ) ;
		') ;
		$Posten->bind_param('iiid', $BestellNr, $Artikel, $Menge, $Preis );
		foreach( $_SESSION['Warenkorb'] as $Eintrag ) {
			$Artikel = $Eintrag->ArtNr ;
			$Menge = $Eintrag->Mng ;
			$Preis = $Eintrag->Prs ;
			$Posten->execute() ;
		}
		// Warenkorb nach Postenübertragung löschen
		unset( $_SESSION['Warenkorb']);
		// Kontrollabfrage zur Anzeige der Bestellung
		$Ergebnis = $Verbindung->query('
			SELECT BestellNr, Datum, Bezeichnung, Menge, posten.Preis
			FROM bestellung INNER JOIN (
					posten INNER JOIN artikel ON posten.Artikel = artikel.ArtikelNr)
					ON bestellung.BestellNr = posten.Bestellung
			WHERE bestellung.BestellNr = "'.$BestellNr.'";
		');
	} else die('Ohne AGB läuft nix!');
...
<div class="container-fluid main"> 
       			<div class="row ">
					<div class="text-center col-md-12 col-xs-2 col-sm-12 col-lg-12 col-xl-10 col-md-offset-0 col-xs-offset-5 col-sm-offset-0 col-lg-offset-0 col-xl-offset-1 blog-main">
                        <section>
                        	<h1>Sie haben soeben bei uns folgende Bestellung aufgegeben:</h1>
                        </section>
                    </div>                        
                </div>
                <div class="row">
                <div class="text-center col-md-12 col-xs-2 col-sm-12 col-lg-12 col-xl-10 col-md-offset-0 col-xs-offset-5 col-sm-offset-0 col-lg-offset-0 col-xl-offset-1 blog-main">
                	<?php echo( zeigeSelect($Ergebnis) );?>
                </div> 
                </div> 
 <!--Main-Content-->
                           <div class="row">
